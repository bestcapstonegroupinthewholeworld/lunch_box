            ...
            {match.url === party.currentRoute ? (
              <div>
                {party.roundsCompleted > 0 ? (
                  <div>
                    <div>
                      <h2>Blue Team Score</h2>
                      <h3>{party.teams[0].score}</h3>
                      <h2>Green Team Score</h2>
                      <h3>{party.teams[1].score}</h3>
                    </div>
                    {party.roundsCompleted === 3 ? (
                      ...
                      <Button
                        ...
                        size="large"
                        onClick={() =>
                          nextRound(party.id, party.game.lunchbox.id)
                        }
                      >
                        Start Next Round
                      </Button>
...
const mapDispatch = (dispatch, { history }) => {
  return {
    ...
    nextRound: (partyId, lunchboxId) => {
      dispatch(nextRound(partyId, lunchboxId, history));
    },
  };
};
